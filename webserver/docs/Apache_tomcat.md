...
### 역할
- Servlet Container: HTTP 요청을 수신하고, 처리하는 역할
- Spring Container: 애플리케이션 비즈니스 로직을 처리하는 역할

### 처리흐름
1. 클라이언트가 HTTP 요청을 보내면, Tomcat 서버가 이 요청을 수신한다. 
2. 이 요청을 Tomcat 서버는 DispatcherServlet(모든 요청을 처리하는 프론트 컨트롤러로, Spring이 자동으로 등록해놓은 서블릿임)으로 전달한다.
3. DispatcherServlet은 이를 분석해서 요청에 맞는 컨트롤러를 찾아 요청을 전달한다.
4. 시브니스 로직을 처리한다. Spring Container에 의해 의존성이 주입된 Service, Repository를 사용하여 처리한다. 
5. 응답을 반환하면 DispatcherServlet에서 Tomcat으로 전달한다. 
...
---
여기서 내가 궁금한 것
- 서블릿을 별도로 생성한다면? 그러니까 별도로 생성해서 구현한다면 어떤 목적으로 하게 될까? 
- 스레드풀을 관리하고, 스레드를 할당하는게 서블릿 컨테이너라고 하는데 Connector가 클라이언트의 요청을 수신하고 처리할 Context에 전달하는 과정까지라고 하면 Connector가 하는 게 아닐까? Connector에서 스레드풀에서 사용할 스레드가 있는지 확인하고 없으면 대기큐, 있으면 스레드 할당해서 Context -> 서블릿컨테이너로 연결하는 역할을 하는게 아닌지?
- 서블릿 컨테이너에서는 웹 요청을 적절한 서블릿으로 전달하는 역할을 한다고 하는데 서블릿을 별도로 생성할일이 많이 없다. 그러면 서블릿이 몇개없는데 서블릿컨테이너에서는 정말 이게 다 인가?
